...
import { ArrowRight, CheckCircle2, KeyRound, ShieldCheck, UserCog } from 'lucide-react';
...
const AcceptStaffInvite: React.FC = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!token) { toast.error('Convite inválido'); return; }
    ...
    setLoading(true);
    try {
      const { data } = await api.post(`/auth/accept-staff-invite/${token}`, { password });
    ...
    <div className="min-h-screen bg-[#f8f3ff] text-slate-950">
      <div className="grid min-h-screen lg:grid-cols-[0.95fr_1.05fr]">
        <section className="relative hidden overflow-hidden bg-slate-950 p-10 text-white lg:flex lg:flex-col lg:justify-between">
          ...
          <div className="relative"><BrandMark inverted /></div>
          <div className="relative max-w-xl">
            ...
            <h1 className="mt-6 text-5xl font-black tracking-[-0.07em]">Bem-vindo à equipe do condomínio.</h1>
            <p className="mt-5 text-base font-semibold leading-8 text-slate-300">
              Crie sua senha para acessar o painel com as permissões do seu cargo.
            </p>
          </div>
        ...
        <section className="flex items-center justify-center px-5 py-10 sm:px-8">
          <div className="w-full max-w-md">
            <div className="mb-8 flex justify-center lg:hidden"><BrandMark /></div>
            <form onSubmit={handleSubmit} className="rounded-[2rem] border border-violet-100 bg-white/95 p-7 shadow-[0_28px_90px_rgba(76,29,149,0.16)] sm:p-9">
              <div className="mb-7">
                ...
                <p className="mt-5 text-[10px] font-black uppercase tracking-[0.26em] text-violet-700">Convite de colaborador</p>
                <h2 className="mt-2 text-3xl font-black tracking-[-0.06em] text-slate-950">Crie sua senha</h2>
                <p className="mt-2 text-sm font-semibold leading-6 text-slate-500">
                  Seu perfil já foi configurado pelo síndico. Defina uma senha para acessar o sistema.
                </p>
              </div>
              ...
              <p className="mt-6 text-center text-sm font-semibold text-slate-500">
                Já tem conta?{' '}
                <Link to="/login" className="font-black text-violet-700 hover:text-violet-800">Entrar</Link>
              </p>
            </form>
          </div>
        </section>
