...
import { LogOut, Loader2, Mail, Settings, ShieldCheck, Trash2, UserCog, UserPlus, GraduationCap, ArrowRight } from 'lucide-react';
...
const SettingsPage: React.FC = () => {
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6 px-4 py-8">
      ...
      {/* Tutorial guiado — apenas síndico/gestão */}
      {isAdmin && (
        <section className="rounded-2xl border border-violet-100 bg-white p-5 shadow-sm">
          <p className="mb-1 text-[10px] font-black uppercase tracking-[0.2em] text-violet-700">Tutorial</p>
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h2 className="text-base font-black tracking-[-0.03em] text-slate-950">Rever tutorial</h2>
                <p className="text-xs font-semibold text-slate-500">Refaça o tour guiado pelas principais telas da plataforma.</p>
              </div>
            </div>
            <button
              type="button"
              onClick={restartTour}
              className="inline-flex items-center justify-center gap-1.5 rounded-xl bg-violet-700 px-4 py-2.5 text-sm font-black text-white transition hover:bg-violet-800"
            >
              Rever tutorial
