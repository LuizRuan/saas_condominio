...
const IssuesPage: React.FC = () => {
  ...
  const { data: issuesData, isLoading: loadingIssues } = useQuery<IssuesQueryResult>({
    queryKey: ['issues', page, LIMIT, debouncedSearch, filterStatus, filterPriority],
    placeholderData: (prev) => prev,
    queryFn: async (): Promise<IssuesQueryResult> => {
  ...
  const paginationTotal = issuesData?.paginationTotal ?? 0;
  const totalPages = issuesData?.totalPages ?? 1;
  const summary = issuesData?.summary ?? null;
  ...
  return (
    <PremiumPage
      title="Ocorrências"
      subtitle="Acompanhe solicitações, registros e atendimentos."
    ...
    >
      <section className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <MetricCard label="Ocorrências" value={summary?.total ?? 0} helper="no condomínio" icon={<MessageSquareText className="h-4 w-4" />} />
        <MetricCard label="Abertas" value={summary?.open ?? 0} helper="aguardando" icon={<AlertTriangle className="h-4 w-4" />} iconClass="bg-orange-100 text-orange-700" valueClassName="text-orange-600" />
        <MetricCard label="Em análise" value={summary?.inProgress ?? 0} helper="em andamento" icon={<Clock3 className="h-4 w-4" />} iconClass="bg-blue-100 text-blue-700" valueClassName="text-blue-700" />
        <MetricCard label="Alta prioridade" value={summary?.highPriority ?? 0} helper="críticas" icon={<Siren className="h-4 w-4" />} iconClass="bg-red-100 text-red-700" valueClassName="text-red-600" />
      </section>
        ...
        <div className="flex flex-col gap-4 border-b border-violet-100/80 px-5 py-5 sm:px-7 xl:flex-row xl:items-end xl:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs font-semibold text-slate-500">
              {debouncedSearch ? `${paginationTotal} resultado(s) encontrados` : 'Solicitações abertas e históricas'}
            </p>
          </div>
        ...
        {items.length === 0 ? (
          <div className="flex flex-col items-center justify-center px-6 py-16 text-center">
            ...
            <h3 className="mt-5 text-lg font-extrabold tracking-[-0.03em] text-slate-950">
              {(summary?.total ?? paginationTotal) === 0 ? 'Nenhuma ocorrência registrada' : 'Nenhum resultado encontrado'}
            </h3>
            <p className="mt-2 max-w-md text-sm font-medium text-slate-500">
              {(summary?.total ?? paginationTotal) === 0 ? 'Crie uma ocorrência interna ou aguarde solicitações dos moradores.' : 'Tente outra busca ou ajuste os filtros.'}
            </p>
